...
- (void)addDownloadWithSession:(NSURLSession *)session URL:(ReceivedGiftInfo *)model begin:(void(^)(NSString *))begin progress:(void(^)(NSInteger,NSInteger))progress complete:(void(^)(NSDictionary *,NSError *))complet {
    // 获取operation对象
    SGDownloadOperation *operation = [self operationWithUrl:[NSURL URLWithString:model.animEffectUrl].absoluteString];
    
    if (operation == nil) {
        
        operation = [[SGDownloadOperation alloc] initWith:model session:session];
