...
- (instancetype)initWith:(ReceivedGiftInfo *)model session:(NSURLSession *)session {
...
//        _gid = model.gid;
        ...
        // 偏好设置里面存储总数据
        _totalSize = [SGCacheManager totalSizeWith:[NSURL URLWithString:model.animEffectUrl].absoluteString];
        
        // 校验
        if (self.currentSize == self.totalSize && self.totalSize != 0 && [[SGCacheManager updatedAtTimeWith:[NSURL URLWithString:model.animEffectUrl].absoluteString] isEqualToString:model.updatedAt]) {
            return nil;
        }
        // 删除之前存在的记录
        if (self.currentSize == self.totalSize && self.totalSize != 0 && ![[SGCacheManager updatedAtTimeWith:[NSURL URLWithString:model.animEffectUrl].absoluteString] isEqualToString:model.updatedAt]) {
